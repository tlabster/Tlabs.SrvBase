...
namespace Tlabs.Identity.Intern {
  ...
  public class SingletonRolesAdministration : IRolesAdministration {
    ...
    readonly UserAdministration.DefaultNormalizer norm= new();

    ///<inheritdoc/>
    public IQueryable<Role> FilteredRoles(QueryFilter filter) {
      var ret= Enumerable.Empty<Role>();
      Tlabs.App.WithServiceScope(prov => {
        var store= prov.GetRequiredService<IDataStore>();
    ...
    static T ReturnFrom<T>(Func<ILookupNormalizer, RoleManager<Data.Entity.Role>, IDataStore, T> doWith) {
      ...
      Tlabs.App.WithServiceScope(prov => {
        ...
        var rmngr= prov.GetRequiredService<RoleManager<Data.Entity.Role>>();
        var store= prov.GetRequiredService<IDataStore>();
        ret= doWith(norm, rmngr, store);
      });
      return ret ?? throw EX.New<InvalidOperationException>("no data for {entity}", typeof(T));
    }

    static List<KeyValuePair<string, Role>> internalAllPersistentRoles() => ReturnFrom((norm, roleMngr, store)
      => { try {
